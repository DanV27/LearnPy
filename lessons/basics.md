---
title: Python Basics
summary: Pick a topic below to learn a fundamental Python concept — one short, focused lesson at a time.
related: data-structures, file-io
---

Welcome to the Python Basics section. Everything here is a building block — you can read it top to bottom or jump straight to whatever you need.

Each lesson is short, has at least one working code example, and links to related concepts. If something is unfamiliar, the orange "Continue exploring" buttons at the bottom of every page will go deeper on demand.

Pick a topic from the grid below to get started.
